...
The package is built around five main functions:
...
- `oag_fetch()` to wrap in a single call building a query, making a
  request, and getting the data from the API
- `oag_parse_object()` to parse an object returned by
  `oag_request()`/`oag_fetch()` and turn it into a `tibble`
...
`oag_fetch()` and request to only return the first record for the type
“publication”. The object returned `oag_fetch()` is of class
`"oag_object"` and has an `"numFound"` attribute containing how many
records for the corresponding filter are in the database.
...
2026. Here, we will use `oag_query()` + `oag_request()` to show the
difference. `oag_request()` returns the results from the query to the
API as a list of length 2 containing the header and the results.

``` r
# Get the the first record of the research product type "dataset", published
# on 15 January 2026.
...
the metadata for the datasets published on 15 January 2026. In the
...
Offset-based paging is limited to small dataset of 10’000 records, with
the numbers of records being defined as the product of the `size` and
`pageSize` options. When the dataset is larger than 10’000 records,
cursor-based paging should be used. The example above can be reproduced
using cursor-based paging by setting `cursor = TRUE` in `oag_options()`:
...
“software” in the “research-product” entity, and use it to fetch data
from the OpenAIRE Graph API.
...
The `oag_parse_object()` function can be used to parse the data in the
objects returned by the OpenAIRE Graph API. You can directly pipe the
results from `oag_fetch()` to `oag_parse_object()` without the need to
specify the entity since the object returned by `oag_fetch()` has an
`"entity"` attribute that is used by `oag_parse_object()` to know how to
parse the data.

``` r
attr(datasets_2026_15_01_cursor, "entity")
#> [1] "research-products"
```

How and what data are parsed by `oag_parse_object()` is based on the
properties of the different object described in the OpenAIRE Graph data
model (see the description of [entities’
properties](https://graph.openaire.eu/docs/category/entities) for more
details).

In the example below, we fetch data on software published in January
2026.

``` r
software_2026_15_01 <- oag_fetch(
  entity = "research-products",
  type = "software",
  fromPublicationDate = "2026-01-15",
  toPublicationDate = "2026-01-15",
...
Then, we parse the whole objects. If you try yourself, you’ll see it is
taking some time. Indeed, some properties may contain large data
requiring more time to be parsed. This may be the case for properties
like “authors” or “organizations”.

``` r
parsed_software <- oag_parse_object(software_2026_15_01)
```

If you do not need all the data to be parsed, you can use the
`selection` argument to select of subset of properties to parse and
return. In the following example, we tell `oag_parse_object()` to only
parse the id, title publisher, and URL of the repository for the
software published in January 2026.

``` r
parsed_software <- oag_parse_object(
  software_2026_15_01,
  selection = c("id", "mainTitle", "publisher", "codeRepositoryUrl")
)
```
